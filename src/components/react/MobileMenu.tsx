...
export default function MobileMenu({ currentPath = '' }: MobileMenuProps) {
  ...
  const menuOverlay = (
    <div
      ref={overlayRef}
      style={{ display: 'none' }}
      className="fixed inset-0 z-[9999] w-full h-[100dvh] bg-[#111513] text-white flex flex-col justify-between overflow-y-auto overscroll-contain select-none"
      role="dialog"
      aria-modal="true"
  ...
  return (
    <div className="lg:hidden">
      ...
      {/* Portal overlay directly to body to avoid containing block issues from sticky/backdrop-filter header */}
